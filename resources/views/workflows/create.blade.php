    ...
    <div class="w-full relative z-10 p-lg overflow-y-auto h-[calc(100vh-4rem)]">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-md mb-lg md:mb-xl">
            <div>
                ...
                <h2 class="font-headline font-bold text-[24px] sm:text-headline-xl text-on-surface">Create New Workflow</h2>
                <p class="text-on-surface-variant text-[14px] sm:text-body-lg mt-xs sm:mt-sm">Add details to initialize a new workflow.</p>
            </div>
        </div>

        <form action="{{ route('workflows.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-md sm:gap-lg">
            @csrf
            ...
            <div class="md:col-span-8 space-y-md sm:space-y-lg">
                ...
                <div class="glass-effect p-lg rounded-xl border border-outline-variant/50 space-y-sm">
                    ...
                    <textarea id="description" name="description" rows="6"
                        class="w-full bg-transparent text-body-lg text-on-surface placeholder:text-on-surface-variant/40 focus:outline-none resize-none leading-relaxed" placeholder="Describe what this workflow does...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-error text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
