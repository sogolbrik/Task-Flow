    ...
    <div class="w-full relative z-10 p-md sm:p-lg overflow-y-auto h-[calc(100vh-4rem)]">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-md mb-lg md:mb-xl">
            <div>
                ...
                <h2 class="font-headline font-bold text-[24px] sm:text-headline-xl text-on-surface">Create New Task</h2>
                <p class="text-on-surface-variant text-[14px] sm:text-body-lg mt-xs sm:mt-sm">Add details to initialize a new task for the current sprint.</p>
            </div>
        </div>

        <form action="{{ route('tugas.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-md sm:gap-lg">
            @csrf
            ...
            <div class="col-span-1 md:col-span-12 mt-lg flex flex-col sm:flex-row items-center justify-between pt-md sm:pt-lg border-t border-outline-variant/50 gap-md sm:gap-0">
